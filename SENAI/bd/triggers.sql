...
insert into Itens_Pedido(pedido_id, pizza_id, quantidade, valor) values(30,1,1, (select valor from Pizzas where pizza_id = 1));

drop trigger if exists tr_atualiza_valor_pedido;
create trigger tr_atualiza_valor_pedido after insert on Itens_Pedido
for each row
update pedidos set valor = (select sum(quantidade * valor) from itens_pedido where pedido_id = )